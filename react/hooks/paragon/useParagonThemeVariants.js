 ...
 */
const useParagonThemeVariants = _ref => {
  let {
    themeVariants,
    currentThemeVariant,
    onComplete,
  ...
  useEffect(() => {
    ...
    Object.entries(themeVariants).forEach(_ref2 => {
      let [themeVariant, value] = _ref2;
      ...
      const createThemeVariantLink = function (url) {
        let {
      ...
      const insertBrandThemeVariantLink = () => {
        ...
        if (value.urls.brandOverride) {
          const brandThemeVariantLink = createThemeVariantLink(value.urls.brandOverride, {
