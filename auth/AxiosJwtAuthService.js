...
class AxiosJwtAuthService {
   ...
   */
  getLoginRedirectUrl() {
    let redirectUrl = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.config.BASE_URL;
  ...
  async ensureAuthenticatedUser() {
    ...
    if (this.getAuthenticatedUser() === null) {
      const isRedirectFromLoginPage = global.document.referrer && global.document.referrer.startsWith(this.config.LOGIN_URL);
  ...
  async hydrateAuthenticatedUser() {
    ...
    if (user !== null) {
      const response = await this.authenticatedHttpClient.get(`${this.config.LMS_BASE_URL}/api/user/v1/accounts/${user.username}`);
