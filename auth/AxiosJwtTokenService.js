...
export default class AxiosJwtTokenService {
  ...
  constructor(loggingService, tokenCookieName, tokenRefreshEndpoint) {
    ...
    this.httpClient.interceptors.response.use(response => response, createRetryInterceptor({
  ...
  refresh() {
    ...
    if (this.refreshRequestPromises[this.tokenCookieName] === undefined) {
      const makeRefreshRequest = async () => {
        ...
        const browserDriftSeconds = responseServerEpochSeconds > 0 ? Math.abs(browserEpochSeconds - responseServerEpochSeconds) : null;
